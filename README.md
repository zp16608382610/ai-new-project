...
Phase 9D 让 `eligible=true` 的案件继续往前走一步:由纯领域 `TreatmentPlanner`(`backend/app/after_sales/treatment.py`,无 SQLAlchemy / 无 LLM)生成结构化 **TreatmentPlan**(action 只能是 REFUND / EXCHANGE / REPAIR,且必须等于用户自己提出的 `requested_action`;`eligible=False` / `None` 或诉求为 UNKNOWN 时不选动作、不建执行型工单),再由 `AfterSalesTreatmentService` 复用既有 `TicketService` 创建 / 复用**售后工单**(`tickets.case_id → after_sales_cases.id`,Alembic 迁移 `8b1f3c5d7e90`),把处理方案写入 `after_sales_cases.collected_information["treatment_plan"]`,案件保持 `PROCESSING`(表示「任务已建立、等待执行」,不是 COMPLETED)。**幂等**:同一案件重复处理只复用工单,不重复创建;**不执行任何业务动作**(无 create_refund / cancel_order / 换货 / 维修),真正执行留给后续 Phase。Demo timeline 新增 `Treatment Plan` / `Ticket Creation` 步骤,`/chat` 可见 Case ID / Eligibility / Treatment Action / Ticket ID / Case status;Evaluation 新增 6 个 case 与 5 项指标(`treatment_plan_accuracy` / `ticket_creation_success` / `ticket_id_presence` / `duplicate_ticket_rate` / `execution_not_triggered`);评测 runner 为每个 case 使用独立的一次性种子库,保证数据集顺序无关。新增 28 例测试,全套 **510 例全绿**;零新增第三方依赖。见 docs/ARCHITECTURE.md §27 与 docs/DECISIONS.md Decision 047。

Phase 9E 让 `eligible=true` 且 `TreatmentPlan.action=REFUND, executable=true` 的售后案件真正走完**执行闭环**，并且**只复用既有安全链**：新增服务层 `AfterSalesExecutionService`（只负责把执行 / 校验结果写回 Case，从不自己执行业务逻辑），由 9D 的 TreatmentPlan 触发既有 **Risk Gate**（`app.risk.RiskEngine` / `RiskPolicy`，沿用既有 LOW / MEDIUM / HIGH / CRITICAL，未新增风险等级）→ 高风险进入既有 **Human Approval**（`ApprovalService` / `approval_requests`；Case 关联通过冻结的 `ToolRequest` `case_id` 快照传递，**没有新建第二套审批模型**）→ 既有 **Tool Executor** → 既有 **`RefundService.create_refund`**（唯一退款实现，没有第二套退款逻辑）→ `RefundRepository`。**Execute ≠ Completed**（Decision 048）：工具调用成功本身**不等于**业务完成——执行后必须由 `BusinessVerifier` 重新读取权威退款记录并校验 `refund_exists` / `order_matches` / `status_pending` / `amount_matches_order_total`，且 `AfterSalesExecutionService` 在写入 `COMPLETED` 前会**再次**按 id 重新读取退款行；取不到退款行（`EXECUTION_REFUND_MISSING`）或没有通过的校验（`EXECUTION_NOT_VERIFIED`）一律拒绝完成。退款金额始终由业务系统（订单总价）决定，用户说「退款 5000 元」也只会得到权威金额（ORD-1003 = ¥199.00），LLM 无法指定金额、也无法绕过 Risk Gate；`EXCHANGE` / `REPAIR` 本阶段没有可执行的业务系统，一律如实记录 `NOT_IMPLEMENTED` / `HUMAN_HANDOFF`（或 `policy_covers_action` 拒绝）并转人工，**绝不伪造**「换货成功 / 维修成功」。案件新增 `PENDING_HUMAN` / `COMPLETED` 状态，成功路径为 `PROCESSING → Execute → Verify → COMPLETED`，执行结果复用 `collected_information` 存储（不新增第二张表、不新增十几个字段）。Demo payload 新增 `execution` / `verification`，timeline 只展示真实发生的 `Risk Gate → Approval → Execute → Verify` 步骤。Evaluation 新增 8 个 case（E1 合法退款 / E2 高额退款 / E3 人工拒绝 / E4 Execute 失败 / E5 Verify 失败 / E6 重复执行 / E7 换货 / E8 维修）与 3 项指标（`completion_correctness` / `approval_gate_correctness` / `duplicate_execution_rate`），全量 **31/31 PASS**。本阶段零新增第三方依赖（无 LangChain / LangGraph / 向量库 / 真实支付），未削弱 9A–9D 既有断言；全套 **533 例全绿**。见 docs/ARCHITECTURE.md §28 与 docs/DECISIONS.md Decision 048。

Phase 9F(A+B) 把「退款诉求 + 商品质量问题」统一进售后链路,并给出第一条低风险自动执行路径:① **统一路由**——同时报告质量问题并要求退款的请求(如「我的耳机左边坏了,订单是 ORD-1003,我要退款。」)进入完整 `AFTER_SALES_CASE` 链路(Investigation → Policy RAG → Eligibility → TreatmentPlan → Ticket → Risk → Execute → Verify),不再误走历史的一次性 `REFUND_TOOL`;② **低风险自动退款**——新增策略 `P-REFUND-LOW-RISK-AUTO`(Decision 049):资格已确认(`eligible=True`)+ 质量问题退款 + 订单 `DELIVERED` + 无在途退款 + 商品可退 + 金额 < 500 时,Risk Gate 判定 `LOW / AUTO_EXECUTE`,再由既有 ToolExecutor → RefundService 执行并 Verify;任一条件缺失或未知一律回落 `HIGH / HUMAN_APPROVAL`,金额 >= 500 仍为 `CRITICAL`;③ **Demo 时钟**——demo seed 的签收时间改为以可注入的 `now` 为锚点相对生成(`seed_demo_orders(session, now=...)`,线上 bootstrap 传真实时钟,测试 / 评测注入固定锚点,见 Decision 050),因此线上 demo 不会因为 seed 日期老化而得到 `REJECTED`,可现场演示「退款成功 → Execute → Verify」。

## Next Phase

**Next: Phase 9F(剩余)– After-Sales Exchange / Repair Execution,not started**

Phase 9F 剩余部分及以后仍需补齐:换货 / 维修的**真实执行**(本阶段二者如实记录为 `NOT_IMPLEMENTED` / `HUMAN_HANDOFF`)、售后金额与库存联动、人工复核流程的扩大。Phase 8 保留规模化评测集(Retrieval / Generation / Agent / Tool / Product 五层)、生产级指标与日志聚合;Phase 3D(知识库管理 + 真实 Embedding / pgvector 实机验证)与 Phase 9 Final Demo(现场彩排 / 验收)仍为 not started,范围独立,可后续单独推进。详细路线见 [docs/DEVELOPMENT_PLAN.md](docs/DEVELOPMENT_PLAN.md)。
