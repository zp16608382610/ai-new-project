...
五层评测(Retrieval / Generation / Agent / Tool / Product)与回归流程、端到端 trace(request → model → state → retrieval → tool → result → answer)与行为审计(ToolResult metadata 已预留 execution_id / duration_ms 等)。另注:Phase 3D(知识库管理 + 真实 Embedding / pgvector 实机验证)仍为 not started,范围独立,可后续单独推进。详细路线见 [docs/DEVELOPMENT_PLAN.md](docs/DEVELOPMENT_PLAN.md)。
...
```text
.
├── backend/            # FastAPI 后端(健康检查 + 数据层 + Mock Business API + 知识库/检索/Agent 层骨架 + MCP:schemas/service/repository/routes/knowledge/retrieval/agent/mcp)
