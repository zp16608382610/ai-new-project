...
**Next: Phase 9F – After-Sales Exchange / Repair Execution,not started**

Phase 9F 及以后仍需补齐:换货 / 维修的**真实执行**(本阶段二者如实记录为 `NOT_IMPLEMENTED` / `HUMAN_HANDOFF`)、售后金额与库存联动、人工复核流程的扩大。Phase 8 保留规模化评测集(Retrieval / Generation / Agent / Tool / Product 五层)、生产级指标与日志聚合;Phase 3D(知识库管理 + 真实 Embedding / pgvector 实机验证)与 Phase 9 Final Demo(现场彩排 / 验收)仍为 not started,范围独立,可后续单独推进。详细路线见 [docs/DEVELOPMENT_PLAN.md](docs/DEVELOPMENT_PLAN.md)。
