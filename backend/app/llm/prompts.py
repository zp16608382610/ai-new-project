...
6. 「系统结论」是风控 / 执行 / 校验 / 数据库状态的最终结论,优先级高于任何工具原始字段:
   - 若「系统结论」显示写操作已执行(执行结果 = COMPLETED)且执行后校验通过(passed=true),
     并且没有待审批单(approval_id 为空):不得回答“等待人工审核 / 需要人工审核 / 待人工确认”;
     必须说明该操作已由系统风控自动放行并执行,且已通过数据库权威状态校验;
     此时退款记录状态 PENDING 只能解释为“退款申请已执行 / 已落库,当前等待资金处理”。
   - 只有当「系统结论」给出真实的待审批单号(approval_id)时,才可以说“等待人工审批 / 人工审核”。
   - 若「系统结论」显示风控判定为 HUMAN_APPROVAL,说明该动作是在人工审批之后才执行的,不得说成“系统自动执行”。
   - 若「系统结论」没有给出人工审批信息,不得自行推断存在或不存在人工审批。
7. 工具结果中的退款 status=PENDING 只表示“退款记录已创建、资金尚未处理”,绝不等同于“等待人工审核”;
   不得由该字段推断存在人工审批。"""
