...
# Phase 9F: the evaluation runner seeds the demo database with this fixed
# anchor (prepare_demo_database(..., now=DEMO_ANCHOR)) and derives every
# reference_time from it, so the after-sales window is reproducible instead of
# depending on the wall clock. ORD-1003 is delivered 2 days before the anchor.
DEMO_ANCHOR = datetime(2026, 9, 11, 12, tzinfo=timezone.utc)
IN_WINDOW = DEMO_ANCHOR.isoformat()                             # 2 days after delivery
OUT_OF_WINDOW = (DEMO_ANCHOR + timedelta(days=37)).isoformat()  # 39 days after
...
EVALUATION_DATASET: tuple[EvaluationCase, ...] = (
    ...
    # ---- Phase 9E/9F: after-sales execution + verification -----------------
    # Phase 9F: ORD-1003 / 199 is the standard, already-verified low-risk refund
    # (LOW -> AUTO_EXECUTE). The reject / failure / verify-failure / duplicate
    # scenarios use the high-value order ORD-1001 / 1299 so they keep the real
    # human-approval path; the high-value case below stays the reference for it.
    EvaluationCase(
        case_id="after-sales-execute-refund",
        category="After-sales / Execution",
        scenario="Standard low-risk refund (199 < 500) -> LOW -> AUTO_EXECUTE -> Execute -> Verify -> COMPLETED with no human approval.",
        ...
        note="Phase 9F: every low-risk fact holds (eligible, quality issue, DELIVERED, returnable, no active refund, 199 < 500), so the refund auto-executes. The amount (199) is still decided by RefundService from the order, never by the LLM.",
    ),
    EvaluationCase(
    ...
    EvaluationCase(
        ...
        note="Phase 9F: the high-value order (1299 -> CRITICAL) keeps a real human decision. Reject must leave the business state untouched: no refund row, no COMPLETED case.",
    ),
    EvaluationCase(
        ...
        note="Phase 9F: the high-value order keeps the human-approval path. The refund backend raises; the case must not claim completion and must not invent an id.",
    ),
    EvaluationCase(
        ...
        note="Phase 9F: the high-value order keeps the human-approval path. Tool success is not completion: the business state re-read fails -> never COMPLETED.",
    ),
    EvaluationCase(
        ...
        note="Phase 9F: the high-value order keeps the first run on the human-approval path; after it completes, RefundService's existing active-refund constraint refuses the second execution.",
    ),
    EvaluationCase(
